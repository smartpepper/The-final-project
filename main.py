...
def dist(x1, y1, x2, y2):
    "считает расстяние между двумя точками"
    return sqrt((x2 - x1)**2 + (y2 - y1)**2)


def dist_line(x0, y0, x1, y1, x2, y2):
    "считает расстояние от точки до линии"
    return abs((y2 - y1) * x0 - (x2 - x1) * y0 + x2 * y1 - x1 * y2) / dist(x1, y1, x2, y2)


def itog(x, y):
    "высчитыет расстояния в см от метки до начала координат"
    rx = dist_line(x[3], y[3], x[0], y[0], x[2], y[2]) * 500 / dist_line(x[1], y[1], x[0], y[0], x[2], y[2])
    ry = dist_line(x[3], y[3], x[0], y[0], x[1], y[1]) * 500 / dist_line(x[2], y[2], x[0], y[0], x[1], y[1])
